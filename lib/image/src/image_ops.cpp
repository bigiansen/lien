...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    {
        return _internal::get_channel_std(data, len, 0);
    }

    std::vector<uint8_t> channel_green(const uint8_t* data, size_t len)
    {
        return _internal::get_channel_std(data, len, 1);
    }

    std::vector<uint8_t> channel_blue(const uint8_t* data, size_t len)
    {
        return _internal::get_channel_std(data, len, 2);
    }

    std::vector<uint8_t> channel_alpha(const uint8_t* data, size_t len)
    {
        return _internal::get_channel_std(data, len, 3);
    }

    namespace _internal
    {
        std::vector<uint8_t> get_channel_std(const uint8_t* data, size_t len, size_t channel_idx)
        {
            debug_assert(len % 4 == 0, "Length must be a multiple of 4!");
            std::vector<uint8_t> result;
            result.resize(len / 4);
            for(size_t i = 0; i < len; i += 4)
            {
                result[i / 4] = data[i + channel_idx];
            }
            return result;
        }
    }
