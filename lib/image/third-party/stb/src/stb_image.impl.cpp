...
// Align to 16byte on SSE2 compatible platforms
        ...
        #define STBI_MALLOC(sz) _aligned_malloc(sz, LIEN_STBI_ALLIGN_MALLOC)
        #define STBI_FREE(h) _aligned_free(h)
        #define STBI_REALLOC(h, sz) _aligned_realloc(h, sz, LIEN_STBI_ALLIGN_MALLOC)
    #else
        #define STBI_MALLOC(sz) std::aligned_alloc(LIEN_STBI_ALLIGN_MALLOC, sz)
